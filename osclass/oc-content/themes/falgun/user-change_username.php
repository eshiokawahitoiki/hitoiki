<?php
    ...
    function custom_meta_title($data){
        return __('Change username', 'falgun');;
    }
...
$(document).ready(function() {
    $('form#change-username').validate({
        ...
        messages: {
            s_username: {
                required: '<?php echo osc_esc_js(__("Username: this field is required", "falgun")); ?>.'
            }
        },
            ...
            cInterval = setInterval(function(){
                $.getJSON(
                    ...
                    function(data){
                        clearInterval(cInterval);
                        if(data.exists==0) {
                            $("#available").text('<?php echo osc_esc_js(__("The username is available", "falgun")); ?>');
                        } else {
                            $("#available").text('<?php echo osc_esc_js(__("The username is NOT available", "falgun")); ?>');
                        }
                    }
                );
            }, 1000);
        ...
        <div class="form-login col-md-offset-2 col-md-offset-right-2">  
            ...
            <form action="<?php echo osc_base_url(true); ?>" method="post" id="change-username">
                <div class="form-row-header"> 
                    <h1 id="form-row-h1"><?php _e('Change username', 'falgun'); ?></h1>
                </div>   
                    <input type="hidden" name="page" value="user" />
                    <input type="hidden" name="action" value="change_username_post" />
                    <div class="control-group">
                        <label class="control-label" for="s_username"><?php _e('Username', 'falgun'); ?></label>
                        <div class="controls">
